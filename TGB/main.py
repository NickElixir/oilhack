        ...
        if last_chat_text:
            ...
            if last_chat_text.lower()=="/start":
                ...
                if len(last_chat_text.lower())>9:
                    ...
                    classes=['message default clearfix','message default clearfix joined']
                    archiv_bot.send_message(last_chat_id, 'Done!')
            elif last_chat_text.lower()[:6]=='/table':
                #links_table = pd.read_csv('./propertys/links_table.csv')
                #links_table.company_name.loc[]
                #answer='./tables/' + last_chat_text[7:] + '.csv'
                word = last_chat_text[7:]
                links_table = pd.read_csv('./propertys/links_table.csv', sep=';', encoding="utf-8")
                ...
                file = open(f'companies/{word}.csv', 'w')
                returnable_df = pd.DataFrame()
                for i in res:
                        ...
                        tmp = infile.readline()
                        tables += ['\n'] + infile.readlines()
                        tmp1 = pd.read_csv(path, sep=';')
                        returnable_df = returnable_df.append(tmp1)

                        infile.close()

                imp = 0
                returnable_df.to_excel(f'companies_excel/{word}.xlsx', index=)
                ...
                new_df = pd.DataFrame(
                    {'Общий бюджет': [returnable_df['Бюджет'].sum()], 'Коэффициент импортозависимости': [imp]})
                new_df.to_excel('testXlsx.xlsx', index=False)
                file.write(tmp)
                for x in tables:
                ...
                print(tables)
            elif last_chat_text.lower()[:6] == '/итоги':
                word = last_chat_text[7:]
